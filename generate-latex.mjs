...
// LaTeX primitives that read the filesystem or shell out. The .tex here is
// generated by the agent from the user's CV *and the job description*, so a
// posting carrying injected markup could otherwise pull an arbitrary local
// file into the PDF the user then emails to a recruiter — a quiet exfiltration
// channel through a document they believe they authored.
//
// pdflatex already gets -no-shell-escape and tectonic disables shell escape by
// default; this closes the file-read primitives those flags don't cover.
// \input{glyphtounicode} is the one legitimate use in the shipped template, so
// it is allowed by exact spelling.
const ALLOWED_INPUT_ARGS = new Set(['glyphtounicode']);

const FORBIDDEN_PRIMITIVES = [
  { re: /\\write18\b/g, name: '\\write18 (shell escape)' },
  { re: /\\immediate\s*\\write\b/g, name: '\\immediate\\write' },
  { re: /\\openin\b/g, name: '\\openin (file read)' },
  { re: /\\openout\b/g, name: '\\openout (file write)' },
  { re: /\\read\b/g, name: '\\read (file read)' },
  { re: /\\catcode\b/g, name: '\\catcode (character class redefinition)' },
  { re: /\\csname\b/g, name: '\\csname (dynamic control sequence)' },
  { re: /\\ShellEscape\b/g, name: '\\ShellEscape' },
  { re: /\\directlua\b/g, name: '\\directlua' },
  { re: /\\usepackage\s*(\[[^\]]*\])?\s*\{[^}]*\b(shellesc|spawn|python|pythontex|minted)\b[^}]*\}/g,
    name: '\\usepackage of a shell-escape package' },
];

// Collect security violations in the .tex source. Returns an array of strings.
function findForbiddenPrimitives(content) {
  const found = [];

  for (const { re, name } of FORBIDDEN_PRIMITIVES) {
    re.lastIndex = 0;
    if (re.test(content)) {
      found.push(`Forbidden LaTeX primitive: ${name}`);
    }
  }

  // \input / \include with anything other than the known-good template include.
  for (const match of content.matchAll(/\\(input|include)\s*\{([^}]*)\}/g)) {
    const arg = match[2].trim();
    if (!ALLOWED_INPUT_ARGS.has(arg)) {
      found.push(`Forbidden \\${match[1]}{${arg}} — only ${[...ALLOWED_INPUT_ARGS].join(', ')} may be included`);
    }
  }
  // Brace-less form: \input /etc/passwd
  for (const match of content.matchAll(/\\input\s+([^\s{][^\s]*)/g)) {
    found.push(`Forbidden \\input ${match[1]} — file inclusion is not permitted`);
  }
...
async function main() {
  ...
  // Security gate first — a .tex that tries to read the filesystem or shell
  // out is rejected before we consider whether it is a well-formed CV.
  const securityIssues = findForbiddenPrimitives(content);
